...
function randomGenerate() {
    ...
    randomButton.addEventListener('click', () => {
        for (let i = 0; i < columns; i++) {
            for (let j = 0; j < rows; j++) {
                currentBoard[i][j].box = floor(Math.random() * 2);
            }
        }
    })
}
...
function boxColor() {
    const jsonColor = JSON.stringify(colorPicker.value())
    if (currentBoard[i][j].box === 1) {
        currentBoard[i][j].color = colorPicker.color()
        currentBoard[i][j].colorForever = jsonColor
    }
    if (currentBoard[i][j].colorForever) {
        fill(currentBoard[i][j].colorForever)
    } else if (!currentBoard[i][j].colorForever) {
        fill(currentBoard[i][j].color) 
    } else if (currentBoard[i][j].box === 0){
            fill(255)
        }
    }
...
// console.log(jsonColor) 
    ...
    for (let i = 0; i < columns; i++) {
        for (let j = 0; j < rows; j++) {
            ...
            if (currentBoard[i][j].box === 0){
                    fill(255) 
            } else if (currentBoard[i][j].box === 1) {             
                currentBoard[i][j].colorForever = jsonColor
                currentBoard[i][j].color = colorPicker.value()
                fill(JSON.parse(currentBoard[i][j].colorForever)) 
            } else if ((currentBoard[i][j].color !== JSON.parse(currentBoard[i][j].colorForever)) && (currentBoard[i][j].box === 1)) {
                fill(currentBoard[i][j].color) 
            }
    ...
    if (keyIsDown(32) && (isInputCustomPattern === false)) {
        if (isKeyboardReset) {
            ...
            currentBoard[keyboardCursorX][keyboardCursorY].box = 1
        ...
        for (let y = 0; y < rows; y++) {
            ...
            for (let i of [-1, 0, 1]) {
                for (let j of [-1, 0, 1]) {
                    ...
                    neighbors += currentBoard[(x + i + columns) % columns][(y + j + rows) % rows].box;
                }
            }

            // Rules of Life
            if (currentBoard[x][y].box == 1 && neighbors < rule1Value.value()) {
                // Die of Loneliness
                nextBoard[x][y].box = 0;
            } else if (currentBoard[x][y].box == 1 && neighbors > rule2Value.value()) {
                // Die of Overpopulation
                nextBoard[x][y].box = 0;
            } else if (currentBoard[x][y].box == 0 && neighbors == rule3Value.value()) {
                // New life due to Reproduction
                nextBoard[x][y].box = 1;
            } else {
                // Stasis
                nextBoard[x][y].box = currentBoard[x][y].box;
            }
        }
    }
...
function mouseDragged() {
    ...
    currentBoard[mouseCol][mouseRow].box = 1;
    fill([random() * 255, random() * 255, random() * 255]);
    stroke(strokeColor);
    rect(mouseCol * unitLength, mouseRow * unitLength, unitLength, unitLength);
    return true
...
function mousePressed() {
    ...
    if (selectPattern.value() !== '-' && isKeyboardReset) {
        ...
        for (let i = 0; i < maxCol; i++) {
            for (let j = 0; j < maxRow; j++) {
                currentBoard[i + floor(mouseCol + 2)][j + floor(mouseRow + 1)].box = patternBoard[i][j]
            }
        }
    } else {
