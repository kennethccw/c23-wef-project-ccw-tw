...
function draw() {
    ...
    for (let i = 0; i < columns; i++) {
        for (let j = 0; j < rows; j++) {
            // fill(currentBoard[i][j].life === 1 && !currentBoard[i][j].foreverColor ? true : colorPicker.value())
            if ((currentBoard[i][j].life === 1) && nextBoard[i][j].foreverColor){
                // fill(currentBoard[i][j].life === 1 ? colorPicker.color() : 255) // determine the box color
                fill(nextBoard[i][j].foreverColor)
                nextBoard[i][j].foreverColor = null
            } else {
                fill(255)
            }
    ...
    for (let x = 0; x < columns; x++) {
        for (let y = 0; y < rows; y++) {
            ...
            if (currentBoard[x][y].life == 1 && neighbors < rule1Value.value()) {
                ...
                currentBoard[x][y].foreverColor = colorPicker.value()
