...
function randomGenerate() {
    ...
    randomButton.addEventListener('click', () => {
        for (let i = 0; i < columns; i++) {
            for (let j = 0; j < rows; j++) {
                currentBoard[i][j] = floor(Math.random() * 2);
            }
        }
    })
}
...
function draw() {
    ...
    for (let i = 0; i < columns; i++) {
        for (let j = 0; j < rows; j++) {
            fill(currentBoard[i][j] === 1 ? colorPicker.color() : 255) // determine the box color
    ...
    if (keyIsDown(32) && (isInputCustomPattern === false)) {
        if (isKeyboardReset) {
            ...
            currentBoard[keyboardCursorX][keyboardCursorY] = 1
...
function generate() {
    ...
    for (let x = 0; x < columns; x++) {
        for (let y = 0; y < rows; y++) {
            ...
            // Rules of Life
            if (currentBoard[x][y] == 1 && neighbors < rule1Value.value()) {
                // Die of Loneliness
                nextBoard[x][y] = 0;
            } else if (currentBoard[x][y] == 1 && neighbors > rule2Value.value()) {
                // Die of Overpopulation
                nextBoard[x][y] = 0;
            } else if (currentBoard[x][y] == 0 && neighbors == rule3Value.value()) {
...
function mouseDragged() {
    ...
    currentBoard[mouseCol][mouseRow] = 1;
    fill([random() * 255, random() * 255, random() * 255]);
    stroke(strokeColor);
    rect(mouseCol * unitLength, mouseRow * unitLength, unitLength, unitLength);
    return true
...
function mousePressed() {
    ...
    if (selectPattern.value() !== '-' && isKeyboardReset) {
        ...
        for (let i = 0; i < maxCol; i++) {
            for (let j = 0; j < maxRow; j++) {
                currentBoard[i + floor(mouseCol + 2)][j + floor(mouseRow + 1)] = patternBoard[i][j]
            }
        }
    } else {
