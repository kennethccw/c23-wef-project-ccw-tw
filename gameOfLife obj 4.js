...
function init() {
    ...
    for (let i = 0; i < columns; i++) {
        for (let j = 0; j < rows; j++) {
            currentBoard[i][j] = {life: 0, colorChanged: false};
            nextBoard[i][j] = {life: 0, colorChanged: false};
        }
    }
...
function randomGenerate() {
    ...
    randomButton.addEventListener('click', () => {
        for (let i = 0; i < columns; i++) {
            for (let j = 0; j < rows; j++) {
                currentBoard[i][j].life = floor(Math.random() * 2);
            }
        }
    })
}
...
function draw() {
    ...
    for (let i = 0; i < columns; i++) {
        for (let j = 0; j < rows; j++) {
            currentBoard[i][j].color = JSON.parse(JSON.stringify(colorPicker.value()))
            if (currentBoard[i][j].color !== colorPicker.value())
            // currentBoard[i][j][`${colorPicker.color()}`]
            fill(currentBoard[i][j].life === 1 && currentBoard[i][j].colorChanged ? colorPicker.value() : currentBoard[i][j].color) // determine the box color
    ...
    if (keyIsDown(32) && (isInputCustomPattern === false)) {
        if (isKeyboardReset) {
            ...
            currentBoard[keyboardCursorX][keyboardCursorY].life = 1
        ...
        for (let y = 0; y < rows; y++) {
            ...
            for (let i of [-1, 0, 1]) {
                for (let j of [-1, 0, 1]) {
                    ...
                    neighbors += currentBoard[(x + i + columns) % columns][(y + j + rows) % rows].life;
                }
            }

            // Rules of Life
            if (currentBoard[x][y].life == 1 && neighbors < rule1Value.value()) {
                // Die of Loneliness
                nextBoard[x][y].life = 0;
            } else if (currentBoard[x][y].life == 1 && neighbors > rule2Value.value()) {
                // Die of Overpopulation
                nextBoard[x][y].life = 0;
            } else if (currentBoard[x][y].life == 0 && neighbors == rule3Value.value()) {
...
function windowResized() {
    ...
    for (let i = 0; i < columns; i++) {
        for (let j = 0; j < rows; j++) {
            if (!currentBoard[i][j]) {
                currentBoard[i][j] = {life: 0};
                nextBoard[i][j] = {life: 0};
            }
        }
    }
}
...
function mouseDragged() {
    ...
    currentBoard[mouseCol][mouseRow].life = 1;
    fill([random() * 255, random() * 255, random() * 255]);
    stroke(strokeColor);
    rect(mouseCol * unitLength, mouseRow * unitLength, unitLength, unitLength);
    return true
...
function mousePressed() {
    ...
    if (selectPattern.value() !== '-' && isKeyboardReset) {
        ...
        for (let i = 0; i < maxCol; i++) {
            for (let j = 0; j < maxRow; j++) {
                currentBoard[i + floor(mouseCol + 2)][j + floor(mouseRow + 1)].life = patternBoard[i][j]
            }
        }
    } else {
...
function createColorArr() {
    colorArr.unshift(colorPicker.value())
    if (colorArr === []) {
